...
void mandarLaBase(baseMandar){

    t_buffer* buffer =buffer_create();

    t_Karnel_Memoria instruccion;

   if(baseMandar !=-1 && baseMandar!=-2){
        instruccion = BASE;
        buffer_pack(buffer,&instruccion,sizeof(t_Karnel_Memoria));
        buffer_pack(buffer,&baseMandar,sizeof(uint32_t));
        stream_send_buffer(socketKernel, instruccion, base);
   }
   if(baseMandar ==-1 ){
